...
flowchart TB
    subgraph INGESTION["1. DATA INGESTION"]
        A1[financial_data.csv] 
        A2[product_data.jsonl]
        A3[review_data.jsonl]
    end

    subgraph PREPROCESSING["2. PREPROCESSING"]
        B1[Parse CSV → DataFrame]
        B2[Stream JSONL → Direct Load]
        B3[Stream JSONL → Direct Load]
        
        B1 --> C1[Clean & Validate\nType Conversion]
        B2 --> C2[Validate JSON\nNo Flattening]
        B3 --> C3[Validate JSON\nNo Flattening]
    end

    subgraph SCHEMA["3. SCHEMA & VALIDATION"]
        D1[Financial Schema\nGreat Expectations]
        D2[Product JSONB Validation]
        D3[Review JSONB Validation]
        D1 --> E[Validation Reports]
        D2 --> E
        D3 --> E
    end

    subgraph FEATURE_ENG["4. FEATURE ENGINEERING"]
        F1[Financial Features:\n- discretionary_income\n- debt_to_income_ratio\n- savings_rate]
        F2[Extract from JSONB:\n- description, details, features]
        F3[Extract from JSONB:\n- title, text]
    end

    subgraph EMBEDDINGS["5. VECTOR EMBEDDINGS"]
        G1["Product Embeddings"]
        G2["Review Embeddings"]
        G1 --> H[sentence-transformers\nall-MiniLM-L6-v2\n384 dimensions]
        G2 --> H
    end

    subgraph DATABASE["6. POSTGRESQL + pgvector"]
        I1[(financial_data\nTraditional Columns)]
        I2[(product_data\nJSONB + Vectors)]
        I3[(review_data\nJSONB + Vectors)]
    end
