...
# ---------------------------------------------------------------------------
# Stub all heavy dependencies before loading the module
# ---------------------------------------------------------------------------
...
_make_stub("ingestion", gcs_loader=_gcs_stub, api_loader=_api_stub)
...
def _load():
    candidates = [
        ...
        os.path.join(PROJECT_ROOT, "src", "ingestion", "run_ingestion.py"),
    ]
...
def _gcs_mocks():
    return (
        ...
        patch.object(M, "load_from_api",  return_value=(_DF_FIN, _DF_PROD, _DF_REV)),
    )
...
def test_run_ingestion_routes_to_gcs_when_data_source_gcs():
...
def test_run_ingestion_routes_to_api_when_data_source_api():
...
def test_run_ingestion_raises_on_unknown_data_source():
...
def test_run_ingestion_propagates_exception():
    with patch.object(M, "DATA_SOURCE", "gcs"), \
         patch.object(M, "load_from_gcs", side_effect=RuntimeError("gcs down")):
...
def test_load_from_gcs_returns_three_dataframes():
    with patch("ingestion.gcs_loader.load_financial_data", return_value=_DF_FIN), \
         patch("ingestion.gcs_loader.load_product_data",   return_value=_DF_PROD), \
         patch("ingestion.gcs_loader.load_review_data",    return_value=_DF_REV):
...
def test_load_from_gcs_raises_on_failure():
    with patch("ingestion.gcs_loader.load_financial_data", side_effect=Exception("gcs error")):
...
def test_load_from_api_returns_three_dataframes():
    with patch("ingestion.api_loader.load_financial_data", return_value=_DF_FIN), \
         patch("ingestion.api_loader.load_product_data",   return_value=_DF_PROD), \
         patch("ingestion.api_loader.load_review_data",    return_value=_DF_REV):
...
def test_load_from_api_raises_on_failure():
    with patch("ingestion.api_loader.load_financial_data", side_effect=Exception("api error")):
...
def test_ingest_financial_task_gcs():
    with patch.object(M, "DATA_SOURCE", "gcs"), \
         patch("ingestion.gcs_loader.load_financial_data", return_value=_DF_FIN):
...
def test_ingest_financial_task_api():
    with patch.object(M, "DATA_SOURCE", "api"), \
         patch("ingestion.api_loader.load_financial_data", return_value=_DF_FIN):
        result = M.ingest_financial_task(**_make_context())
        assert result["status"] == "success"
...
def test_ingest_review_task_gcs():
    with patch.object(M, "DATA_SOURCE", "gcs"), \
        ...
        assert result["status"] == "success"
        assert result["records"] == len(_DF_REV)
